...
public class FileManagerWindow : ApplicationWindow {
    ...
    private int sort_mode = 0; // 0: name, 1: size
    ...
    private void toggle_sort(int mode) {
        if (sort_mode == mode) {
    ...
    private void sort_records_in_place(string[] records) {
        for (int i = 0; i < records.length; i++) {
            for (int j = i + 1; j < records.length; j++) {
                if (compare_records(records[j], records[i]) < 0) {
                    var tmp = records[i];
                    records[i] = records[j];
                    records[j] = tmp;
                }
